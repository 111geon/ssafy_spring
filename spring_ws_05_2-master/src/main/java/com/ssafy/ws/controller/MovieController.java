...
public class MovieController {
	 ...
	 */
	@PostMapping("/regist")
	public String doRegist(Model model, @ModelAttribute Movie movie, @RequestPart(required = false) MultipartFile file) throws IllegalStateException, IOException {
		String fileName = file.getOriginalFilename();
	    String timestampedFileName = String.valueOf(System.currentTimeMillis()) + "-" + fileName;
	    
		movie.setImg(fileName);
	    movie.setOrgImg(timestampedFileName);
	    
	    String uploadPath = servletContext.getRealPath("/resources/upload");
	    
	    if(!new File(uploadPath).exists()) new File(uploadPath).mkdir();
	    
	    File target = new File(uploadPath, timestampedFileName);

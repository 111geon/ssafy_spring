...
public class MovieController {
	...
	public String doLogin(@ModelAttribute User user, HttpSession session, Model model) {
		...
		if (selectedUser != null && user.getPass().equals(selectedUser.getPass())) {
			session.setAttribute("loginUser", selectedUser);
		} else {
			model.addAttribute("msg", "사용자 정보가 옳지 않습니다.");
		}
	...
	public String showList(Model model) {
		//코드작성
		model.addAttribute("movies", mService.selectAll());
		return "list";
	}
	 ...
	 */
	@PostMapping("/regist")
	public String doRegist(@ModelAttribute Movie movie, Model model, MultipartFile file) throws IllegalStateException, IOException {
		...
		String img = String.valueOf(System.currentTimeMillis()) + "-" + orgImg;
		movie.setImg(img);
		movie.setOrgImg(orgImg);
		
		model.addAttribute("movie", movie);
		mService.insert(movie, file.getBytes());
